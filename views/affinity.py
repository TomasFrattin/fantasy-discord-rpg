    ...
    async def callback(self, interaction: discord.Interaction):
        if str(interaction.user.id) != self.view_obj.user_id:
            return await interaction.response.send_message(
                "❌ No podés elegir afinidad para otro jugador.", ephemeral=True
            )
...
class ConfirmarAfinidad(View):
    ...
    async def confirmar(self, interaction: discord.Interaction, button: Button):
        if str(interaction.user.id) != self.user_id:
            return await interaction.response.send_message(
                "❌ No podés confirmar por otro jugador.", ephemeral=True
            )
    ...
    async def cancelar(self, interaction: discord.Interaction, button: Button):
        ...
        await interaction.response.edit_message(
            content="🔁 Volvé a elegir tu afinidad:", view=nueva_view
        )
